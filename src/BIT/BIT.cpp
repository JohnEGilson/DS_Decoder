...
void BIT::parse(std::vector<uint8_t> data) {
	...
	int chsign= data[n]*256 + data[n+1];
	if (chsign > 32767) chsign = chsign - 65536;
	pOFFset = 0.00125 * ( chsign ); // raw count = 1/800?
    n += 2;
...
//Check for Engineering version
    if ( EngVer >= 6 ) {
	RH = data[n]*256 + data[n+1];
	n += 2;
	CTDsn = data[n]*256 + data[n+1]; 
    }
}
