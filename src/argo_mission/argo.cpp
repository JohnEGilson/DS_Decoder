...
void Argo_Mission::Decode(std::vector<uint8_t> &data) {
  ...
  fall2prof_time_max = (int)((data[10]<<8) + data[11]);
  ...
  if ( firmware_version >= 2 ) {
    TurnAround_timeTurn = (data[36]<<8) + data[37];
    TurnAround_timeCheck = (data[38]<<8) + data[39];
  }
  // CTD gains and offsets used for CTD Discrete,CTD_Raw,CTD Binned, CTD Drift profiles as well as engineering drift averages
  for(const auto &profile : std::list<std::string>({"CTD_Discrete","CTD_Discrete_Descend","CTD_Binned","CTD_Binned_Descend","CTD_Raw","CTD_Raw_Descend","CTD_Drift"})) {
