...
void profile::insert(std::vector<uint8_t> data) {
	...
	int sensor_id = (data[0] & 0xF0 ); //int sensor_id = data[0];
	int segment;
	if ( sensor_id <= 112 ) { // if profile divided into 4
	  segment = (data[0] & 0x0F ) % 4; 
	} else if (( data[0] & 0x0F ) == 15 )  { // The CTD Drift has 1 more segment possible than other (6 versus 5)
          segment = 6;
	} else {
	  segment = (data[0] & 0x0F ) % 5; 
	}
	int data_id = data[0] - segment; //int data_id = data[3] & 0x0F;
	int pro = 0; //int pro = data[4];

        //std::cout << "profile_insert stats " << sensor_id << " " << segment << " " << data_id << std::endl;
	char profile_key[7];
