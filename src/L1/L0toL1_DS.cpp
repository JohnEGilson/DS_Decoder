...
int float_type = 1; //default to DS for starters (cycle -1,0 do NOT have ARGO_Mission)
float float_version = 2.2; //default to latest DS  for starters (cycle -1,0 do NOT have ARGO_Mission)
std::string L1_path;
std::string L0_path;
std::string SBD_path;
int rest=3;  //default temperature resolution
int resp=1;  //default pressure resolution
...
int main( int argc, char* argv[] ) {
  ...
  missname = ds_path + "/data/" + padsn + "/" + padsn + "_mission.json"; //path of specific mission json
  configname = ds_path + "/data/" + padsn + "/modified_" + padsn + "_config.json"; //path of specific config json
  floatconfig = ds_path + "/data/" + padsn + "/" + padsn + "_config.json"; //path of specific config json which lists ECO/OCR channel names ::named the same as DS
  L0_path = ds_path + "/data/" + padsn + "/json/L0"; //path of original L0 json
  L1_path = ds_path + "/data/" + padsn + "/json/L1"; //path of new L1 json
  SBD_path = ds_path + "/data/" + padsn + "/json/SBD"; //path of possible SBD json
  // LOAD mission.json template for units/description
  if (!std::filesystem::exists(missname)) {
     missname = ds_path + "/config/mission.json"; //path of default mission json
  }
  ...
  if (!std::filesystem::exists(configname)) {  //check to see if float specific "modified_" config exists
    configname = ds_path + "/config/config.json"; //path of default original L0 json
  }
...
// Have now collectioned EngVer...open Engineering_template
  Dianame = ds_path + "/config/diagnostic_" + std::to_string(initial_EngVer) + ".json";
  Engname = ds_path + "/config/Engineering_Data_" + std::to_string(initial_EngVer) + ".json";
  ...
  std::cout << "finished template" << std::endl;
	  ...
	  json_file >> Doc_L0; // initialize json object with input from oringal file
	    ...
	    std::string desc="";
            ...
            if ( Doc_L0[ "Engineering_Data" ].contains("SubCycle") ) {  //in order to apply must get subcycle from engineering
              ...
              ModOneConfig("Zpark" + std::to_string(subcyc),Doc_L0.at("ARGO_Mission")["drift_target_dbar"],unit,desc);
	      ModOneConfig("Rise" + std::to_string(subcyc),Doc_L0.at("ARGO_Mission")["max_rise_minute"],unit,desc);
              ModOneConfig("Fall" + std::to_string(subcyc),Doc_L0.at("ARGO_Mission")["max_fall_to_profile_minute"],unit,desc);
	    }
...
//		            }
                  case 'L': { // V0.6 and after
                    config_strings.push_back("CCzero");
                    config_strings.push_back("CC_km");
                    config_strings.push_back("CCC_CF");
                    config_strings.push_back("ccCOR");
	    	    break;
		            }
                  case 'M': { //"M" has different application on ascent/descent
		    Cb = command.substr(2,1); // If KA versus K
		    if ( Cb == "A" ) {
                      config_strings.push_back("AdZ_1");
                      config_strings.push_back("AdZ_2");
                      config_strings.push_back("AdZ_3");
		    } else {
		    ...
		    break;
	        	...
	        	break;
                  ...
                  case '7': { //"7" has different application on ascent/descent
		    Cb = command.substr(2,1); // If 7A versus 7
		    if ( Cb == "A" ) {
                      config_strings.push_back("AZ1");
                      config_strings.push_back("AZ2");
                      config_strings.push_back("AZ3");
		    } else {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
void rewrite_json(std::string filename, const json &Doc_L0) { //sends in L0 json filename
...
// choose GPS positions that were from THIS cycles surface interval...save to a new json GPSnew
	  ...
	  for (const auto& row : matrix) {
	    const auto& gpstype = row.at("description"); 
      	    if ( ( gpscy == ( cyn + 1 ) && gpstype == "GPS_START" ) | ( gpscy == cyn ) ) { //Keep both positions transmitted latest cycle for descending profile
...
// determine if there are any CONFIG that relate to whether data is transmitted or not thus "Missing"
	...
	int PROup = 3; //presume the Deep profiles and ascent and descent
...
//	  std::cout << SBck[0] << " Send GPS/Eng/Argo" << std::endl;
	...
	if ( RunMission.contains("UNBINd") ) {
  	...
  	if ( RunMission.contains("PROup") ) { //not always included
	  PROup = RunMission["PROup"]["config_value"].get<int>(); //store the CONFIG of profile up/down collection
	} else { //assume doesn't have capability of profiling on ascent
          PROup = 0;
	}
...
//	  const auto& matrix = Doc_L0.at("GPS");
	...
	if ( Doc_L0.contains(std::string{ "CTD_pass_through" }) ) {
          //fout << "  \"CTD_pass_through\": " << Doc_L0.at("CTD_pass_through");
          fout << "," << std::endl; //finish the previous block
          fout << "  \"" << "CTD_pass_through\": [" << std::endl;
          const auto& matrix = Doc_L0.at("CTD_pass_through");
          int i=0;
          for (const auto& row : matrix) {
            i++;
            fout << "    ";
...
//          ress=determine_resolution(Doc_L0.at("ARGO_Mission")["ctd_psal"]["gain"]);
	  ...
	  fout << "    \"max_rise_minute\": " << Doc_L0.at("ARGO_Mission")["max_rise_minute"] << "," << std::endl;
          ...
          fout << "    \"target_surface_second\": " << decimal(Doc_L0.at("ARGO_Mission")["target_surface_second"],4,0) << "," << std::endl;
...
//          std::string status = Doc_L0["BIT"]["status"];
	  ...
	  if (Doc_L0["BIT"]["status"] == "Beacon") {
	     ...
	     write_BIST( fout, "BIT", "pOFFset" , Doc_L0 ,5,resp,0);
	     write_BIST( fout, "BIT", "cpu_voltage" , Doc_L0 ,5,2,0);
	     ...
	     write_BIST( fout, "BIT", "interrupt_id" , Doc_L0 ,1,0,0);
             if ( Doc_L0["BIT"].contains("CTDsn") ) { // Firmware version 1.1 and beyond
               fout << "    \"SBE_response\": " << Doc_L0["BIT"]["SBE_response"] << "," << std::endl;
	       write_BIST( fout, "BIT", "RH" , Doc_L0 ,2,0,0);
	       write_BIST( fout, "BIT", "CTDsn" , Doc_L0 ,5,0,1);
	     } else {
               fout << "    \"SBE_response\": " << Doc_L0["BIT"]["SBE_response"] << std::endl;
	     }
          }
	...
	if ( Doc_L0.contains(std::string{ "Fall" }) || ( SBck[3] == 1 && !isBEACON && !isBIST && !isDPLY ) ) {
	  ...
	  for (const auto& row : matrix) {
             ...
             long long seconds_result = get_seconds_since_1950(result);
             fout << "\"JULD\": " << std::fixed << std::setprecision(5) << std::setw(10) << seconds_result/86400. << ", ";
	     int iTime = row.contains(std::string{"absolute_pressure"});
             if ( iTime ) {
               fout << "\"absolute_pressure\": " << decimal(row["absolute_pressure"],7,resp) << ", ";
	     }
...
// Note: in BEACON MODE, Engineering data is saved to "BIT" within the json
	...
	if ( Doc_L0.contains(std::string{ "SBE_Error_Data" }) ) {
  	  fout << "," << std::endl; //finish the previous block
  	  fout << "  \"SBE_ERROR_Data\": {" << std::endl;
	  if ( Doc_L0.contains(std::string{ "SBE_Error_Data" }) ) {
	    const auto& matrix = Doc_L0.at("SBE_Error_Data");
            int i = 0;
	    for (const auto& loop : matrix.items()) {
  	      i++;
	      std::string name = "\"" + loop.key() + "\"";
...
// output "Mission" no matter what
	...
	for (const auto& loop : RunMission.items()) {
	  ...
	  // for some CONFIG that are disabled within firmware (e.g. Deep float Ice with version prior to 0.8) mark -1
	  if ( name == "\"Ice_Mn" & std::round(10*float_version) <= 7 ) {
	    ss << -1;
	    sc << -1;
          } else {
            ss << std::fixed << std::setprecision(prec) << loop.value()["value"].get<double>();
	...
	if ( Doc_L0.contains(std::string{ "Pump" }) || ( SBck[4] == 1 && !isBEACON && !isBIST) ) {
	  ...
	  if ( Doc_L0.contains(std::string{ "Pump" }) ) {
	    ...
	    for (const auto& row : matrix) {
	     ...
	     iTime = row.contains(std::string{"absolute_pressure"});
             if ( iTime ) {
               fout << "\"absolute_pressure\": " << decimal(row["absolute_pressure"],7,resp) << ", ";
	     }
	     iTime = row.contains(std::string{"elapsed_minute"});
             if ( iTime ) {
               fout << "\"elapsed_minute\": " <<  decimal(row["elapsed_minute"],6,0) << ", ";
	     }
	...
	if ( Doc_L0.contains(std::string{ "CTD_Raw_Descend" }) || ( PROup != 1 && SBck[1]==1 && writeRawCTD && !isBEACON && !isBIST && !isDPLY) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Raw_Descend" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Raw_Descend" }) ) {
	    write_CTD( fout, "CTD_Raw_Descend" , Doc_L0, 1 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Raw_Descend"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor;
	    write_Config( fout, SndCTD, "CTD_Raw_Descend" );
	  }
	  fout << "  ]";
	}

	if ( Doc_L0.contains(std::string{ "CTD_Raw" }) || ( ( PROup == 1 || PROup == 3 ) && SBck[1]==1 && writeRawCTD && !isBEACON && !isBIST ) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Raw" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Raw" }) ) {
	    write_CTD( fout, "CTD_Raw" , Doc_L0, 1 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Raw"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor;
	...
	if ( Doc_L0.contains(std::string{ "CTD_Binned_Descend" }) || ( PROup != 1 && writeBinCTD && !isBEACON && !isBIST && !isDPLY) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Binned_Descend" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Binned_Descend" }) ) {
	    write_CTD( fout, "CTD_Binned_Descend" , Doc_L0, 0 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Binned_Descend"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor;
	    write_Config( fout, SndCTD, "CTD_Binned_Descend" );
	  }
	  fout << "  ]";
	}

	if ( Doc_L0.contains(std::string{ "CTD_Discrete_Descend" }) || ( PROup != 1 && writeBinCTD && !isBEACON && !isBIST && !isDPLY ) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Discrete_Descend" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Discrete_Descend" }) ) {
	    write_CTD( fout, "CTD_Discrete_Descend" , Doc_L0, 0 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Discrete"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor; time,P,T,S
	    write_Config( fout, SndCTD, "CTD_Discrete_Descend" );
	  }
	  fout << "  ]";
	}
	...
	if ( Doc_L0.contains(std::string{ "CTD_Drift" }) || ( SBck[5]==1 && writeDriftCTD && !isBEACON && !isBIST && !isDPLY ) ) {
	  ...
	  if ( Doc_L0.contains(std::string{ "CTD_Drift" }) ) {
	    write_CTD( fout, "CTD_Drift" , Doc_L0 , 2 );
            ...
            size_t countItems = config_template["prof"]["CTD_Drift"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor;
	    write_Config( fout, SndCTD, "CTD_Drift" );
	  }
	  fout << "  ]";
	}
	if ( Doc_L0.contains(std::string{ "CTD_Binned" }) || ( ( PROup == 1 || PROup == 3 ) && writeBinCTD && !isBEACON && !isBIST ) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Binned" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Binned" }) ) {
	    write_CTD( fout, "CTD_Binned" , Doc_L0, 0 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Binned"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor;
	...
	if ( Doc_L0.contains(std::string{ "CTD_Discrete" }) || ( ( PROup == 1 || PROup == 3 ) && writeBinCTD && !isBEACON && !isBIST ) ) {
  	  fout << "," << std::endl; //finish the previous block
      	  fout << "  \"" << "CTD_Discrete" << "\": [" << std::endl;
	  if ( Doc_L0.contains(std::string{ "CTD_Discrete" }) ) {
	    write_CTD( fout, "CTD_Discrete" , Doc_L0, 0 );
	  } else {
            size_t countItems = config_template["prof"]["CTD_Discrete"].size();
	    std::vector<int> SndCTD(countItems,1); //Variables sent by CTD sensor; time,P,T,S
	    write_Config( fout, SndCTD, "CTD_Discrete" );
	  }
	  fout << "  ]";
	}
...
void write_CTD (std::ofstream& fout, std::string varname, const json &Doc_L0c, int chan) {
	...
	if ( Doc_L0c.contains(varname) ) {
	  ...
	  for (const auto& row : matrix) {
             ...
             fout << "    { ";
             for (auto const& [keyr,valr] : row.items()) {
      	       if ( keyr == "elapsed_second" ) {
                     fout << "\"elapsed_second\": " << decimal(row["elapsed_second"],6,0) << ", ";
	       }
	         ...
	         tPTS[0]=2;
      	       ...
      	       if ( keyr == "absolute_pressure" ) {
                     fout << "\"absolute_pressure\": " << row["absolute_pressure"] << ", ";
	       }
...
//	     std::cout << i << " " << matrix.size() << std::endl;
     	     if ( i == matrix.size() ) {
               fout <<  " }" << std::endl;
	     } else {
               fout <<  " }," << std::endl;
...
#include <iterator>
	...
	size_t j = 0;
	size_t k = 0;
        for (const auto& loop : config_template["prof"][varname].items()) { // loop over config.json OR modified_XXXX_config.json in data directory
	       j++;
