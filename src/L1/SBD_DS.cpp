...
bool write_IrrPos(const std::string& output_path, const WeightedResult& record, bool is_Size0) {
    ...
    if ( !is_Size0 ) {
      ...
      if (std::round(record.error) == 99999 ) {
          ...
          outfile << "\"POSITION_ERROR_ESTIMATED\": " << (std::round(record.error*1000.)) << ",";
...
int main( int argc, char* argv[] ) {
 ...
 //process the argments 
    ...
    std::string output_json = ds_path + "/data/" + padsn + "/json/SBD";
    ...
    std::string metafile = ds_path + "/data/" + padsn + "/" + padsn + "_meta.json"; // Must get SN_meta.json to get AOMLid && transmission ID
    if (std::filesystem::exists(metafile)) {  //check to see if float specific meta.json exists; if not exit
      ...
      if ( Meta.contains("DAC_ID_NUMBER") && Meta.contains("TRANSMISSION_ID_NUMBER") ) {
        ...
        SBD_for_file = ds_path + "/data/" + padsn + "/json/L0/" + padaoml + "_" + padtrans + "_L0_" + padcy + ".json";  //name of L0 json we are processing
        output_json = ds_path + "/data/" + padsn + "/json/SBD/" + padaoml + "_" + padtrans + "_SBD_" + padcy + ".json";  //name of SBD json we will be writing
...
// Define the surface interval
	...
	seconds_L0_json_end = seconds_L0_json_start + L0_json["ARGO_Mission"]["target_surface_second"].get<int>();
        if ( NearSurface >= 1 ) { // Add time to ascend from drift
          seconds_L0_json_end = seconds_L0_json_end + 60 * ( L0_json["ARGO_Mission"]["max_rise_minute"].get<int>() );
        }
        if ( NearSurface >= 2 ) { // Add time to drift 
        ...
        if ( NearSurface >= 3 ) { // Add time to descend to profile
          seconds_L0_json_end = seconds_L0_json_end + 60 * ( L0_json["ARGO_Mission"]["max_fall_to_profile_minute"].get<int>() );
        }
      } else {
...
// Moving on to the hex files
    ...
    fs::path hexpath = ds_path + "/data/" + padsn + "/hex";  //path for this floats hex files
