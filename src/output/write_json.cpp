...
//    log(fmt::format("+ Creating float {} json/L1 subdirectory",sn));
  ...
  if (!std::filesystem::exists(metapath)) {
    string metadefault = DS_PATH + "/config/default_meta.json";
...
//    log(std::format("+ Create Float {}_meta.json with default values",sn));
  ...
  if (!std::filesystem::exists(floatconfig)) {
    string configdefault = DS_PATH + "/config/default_float_config.json";
...
//    log(std::format("+ Create Float {}_meta.json with default values",sn));
 // }
  ...
  std::map<string,string> pass_through = {{"CTD",sensor_info.ctd_info}};
  std::stringstream ss;
  std::string line;
      ...
      while (std::getline(ss,line)) {
        if (line.size()) {
          if (!first1)
            fout << "," << std::endl;
          first1 = false;
	  //check to see if there is a ; within a line, if so break it
	  size_t pos = line.find(';');
	  if ( pos != std::string::npos && pos < line.size()-2 ) {
            fout << "    \"" << line.substr(0,pos+1) << "\",";
            fout << std::endl << "    \"" << line.substr(pos+1,line.size()) << "\"";
	  } else {
            fout << "    \"" << line << "\"";
	  }
        }
  ...
  if (argo.received) {
    ...
    fout << "    \"max_fall_to_profile_minute\": " << decimal(argo.fall2prof_time_max,4,0) << "," << std::endl;
  ...
  if (bit.received) {
    ...
    fout << "    \"pOFFset\": " << bit.pOFFset << "," << std::endl;
    ...
    if ( bit.EngVer == 6 | bit.EngVer == 7 ) {
      fout << "    \"SBE_response\": \"" << bit.SBE_response << "\"," << std::endl;
      fout << "    \"RH\": " << bit.RH << "," << std::endl;
      fout << "    \"CTDsn\": " << bit.CTDsn << std::endl;
    } else if ( bit.EngVer >= 3 & bit.EngVer <= 5 ) {
      fout << "    \"SBE_response\": \"" << bit.SBE_response << "\"" << std::endl;
    } else {
      fout << " NO EngVer defined in BIT " << std::endl;
    }
    fout << "  }";
  }
  ...
  if (beacon.received) {
    ...
    if ( beacon.EngVer == 6 | beacon.EngVer == 7 ) {
      fout << "    \"abortFlag\": " << beacon.abortFlag << "," << std::endl;
      fout << "    \"RH\": " << beacon.RH << std::endl;
    } else if ( beacon.EngVer >= 3 & beacon.EngVer <= 5 ) {
      fout << "    \"abortFlag\": " << beacon.abortFlag << std::endl;
    } else {
      fout << " NO EngVer defined in Beacon " << std::endl;
    }
    fout << "  }";
  }
  ...
  std::map<int,std::string> dive_phase = { 
    {0,"Profile sampling transition"},{1,"Start of sink"},{2,"Pump 2 target"},{3,"Seek"},{4,"Drift begin"},{5,"Drift seek"},{6,"Turnaround"},
    {7,"Profile start"},{8,"Profile end"},{9,"End of Sink"},{10,"Sinking"},{11,"Drifting"},{13,"Ascending"},
    {14,"Reached surface"},{15,"Ice turnaround"} };


  //int startFall=0; //init the reference for Pump Time
  //int startRise=0; //init the reference for Pump Time
  if (fall.Scan.size()) {
    ...
    for ( auto s : fall.Scan ) {
      ...
      if (pump.version == 2) { 
        fout << "\"absolute_pressure\": " << decimal(s.presQ,7,1) << ", ";
      }
      fout << "\"phase\": " << decimal(s.phase,2,0) << ", ";
      //if ( s.phase == 1 ) {
      //      startFall = s.t_cnt;
              //std::cout << "startFall " << startFall << std::endl;
      //}
      //if ( s.phase == 4 ) { //or is it the final phase==3?
      //      startRise = s.t_cnt;
              //std::cout << "startRise " << startRise << std::endl;
      //}
      switch(fall.version) {
        //case 1:  fout << "\"description\": \"" << dive_phase_v1[s.phase] << "\" }"; break;// firmware v10.2+
        default: fout << "\"description\": \"" << dive_phase[s.phase] << "\" }"; break;
      }
    }
    fout << std::endl;
    fout << "  ]";
  ...
  // ====== SBE_Error PARAMETERS ==========
  if (sbe_data.list.size()) {
    fout << "," << std::endl;
    fout << "  \"SBE_Error_Data\": {" << std::endl;
    first1 = true;
    for( auto p : sbe_data.list ) {
      if (!first1)
        fout << "," << std::endl;
      first1 = false;
      fout << "    " << std::setw(12) << p.name << ": { ";
      fout << "\"value\": " << std::setw(1) << p.val << ", ";
      //fout << "\"unit\": " << std::setw(6) << p.unit << ", ";
      fout << "\"description\": \"" << p.desc << "\" }";
    }
    fout << std::endl;
  ...
  if (pump.Scan.size()) {
    ...
    for( auto s : pump.Scan ) {
      ...
      //if (pump.version == 1) { 
        //ptime start_time;
      	//if ( s.phase >= 7) {
	//  start_time = ptime(date(2000,1,1),seconds(startRise));
	//} else {
	//  start_time = ptime(date(2000,1,1),seconds(startFall));
	//}
        //s.time = start_time + seconds(s.time_sc);
        //fout << "\"TIME\": \"" << date_format(s.time,config["DATE_FORMAT"]) << "\", ";
      //}
      fout << "\"PRES\": " << decimal(s.pres,7,prec) << ", ";
      if (pump.version == 2) { 
        fout << "\"absolute_pressure\": " << decimal(s.presQ,7,prec) << ", ";
      }
      if (pump.version >= 1) { 
        fout << "\"elapsed_minute\":" << std::setw(7) << decimal(s.time_sc,5,0) << ", ";
        //fout << "\"elapsed_minute\":" << decimal(s.time_sc,7,0) << ", ";
      }
  ...
  for (auto &[pname,vdict] : config["prof"].items()) {
    ...
    // columns
    first1 = true;
    int loopStart=0;  //default to as packed
    int loopEnd= prof[pname].size;
    int loopDir= 1;
    if ( pname == "CTD_Raw_Descend" | pname == "CTD_Binned" ) { //flip from what the float packs (surface to deep)
      loopStart=prof[pname].size - 1;
      loopEnd= -1;
      loopDir= -1;
    }
    if ( ( argo.firmware_version > 0 && argo.firmware_version < 0.85 ) &&  pname == "CTD_Raw" )  { //flip V0.8 from what the float packs (surface to deep)
      loopStart=prof[pname].size - 1;
      loopEnd= -1;
      loopDir= -1;
    }
    if ( ( argo.firmware_version > 0 && argo.firmware_version < 0.85 ) &&  pname == "CTD_Raw_Descend" )  { //flip V0.8 from what the float packs (surface to deep)
      loopStart=0;
    ...
    for ( int i = loopStart; i != loopEnd; i=i+loopDir ) {
      ...
      for (auto & [var,vatts] : vdict.items()) {
		...
		if ( var == "elapsed_second" ) { // DS has time within profile
	            //fout << "\"" << var << "\": " << date_format(date(2000,1,1)prof[pname][var][i],config["DATE_FORMAT"]);
