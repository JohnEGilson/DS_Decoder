...
void Fall_Data::Decode( std::vector<uint8_t> data) {
    ...
    for( unsigned int i = 7; i < nbyte-1; i+=iwidth) {
        s.index = i;
		...
		//}
		s.t_cnt = (data[i]<<8) + data[i+1]; //DS has 10s resolution                   // original S2BGC firmware
		s.time = start_time + seconds(10*s.t_cnt);  
		s.t_cnt = secs + s.t_cnt; //save the raw time counts with startime included 
