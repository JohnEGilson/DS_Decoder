...
std::ostream & operator << ( std::ostream &os, hexfile &h) {
	...
	for (auto & [PID,m] : h.messages) {
		...
		for (auto p : m.packets) {
			...
			int data_id = p.data[0]; 
                        //Split of CTDRaw and Drift
			if ( (( p.data[0] & 0xF0 ) >> 4 ) >= 9 && (( p.data[0] & 0xF0 ) >> 4) <= 11 ) { 
			        int segment = ( p.data[0] & 0x0F ) % 5; //JG
			        data_id = p.data[0] - segment; //JG
		                //std::cout << "Processing multiuse 5 key " << data_id << " " << segment << std::endl;
			}
                        //Split of CTDBinned Ascent/Descent & CTD_Discrete Ascent/Descent
			if ( (( p.data[0] & 0xF0 ) >> 4 ) >= 1 && (( p.data[0] & 0xF0 ) >> 4) <= 3 ) { 
			        int segment = ( p.data[0] & 0x0F ) % 4; //JG
			        data_id = p.data[0] - segment; //JG
			}
                        //Split of Discrete Time Ascent/Descent
			if ( (( p.data[0] & 0xF0 ) >> 4 ) == 7 ) { 
			        int segment = ( p.data[0] & 0x0F ) % 4; //JG
			        data_id = p.data[0] - segment; //JG
			}
                        //Split of Quartzdyne Discrete and Rise
			if ( ( ( ( p.data[0] & 0xF0 ) >> 4 ) == 5 ) && ( p.data[0] & 0x0F ) >= 8 ) { 
			        int segment = ( p.data[0] & 0x0F ) % 4; //JG
			        data_id = p.data[0] - segment; //JG
			}
                        //Split of Quartzdyne Drift
			if ( ( ( ( p.data[0] & 0xF0 ) >> 4 ) == 13 ) && ( p.data[0] & 0x0F ) >= 10 && ( p.data[0] & 0x0F ) != 15 ) { 
			        int segment = ( p.data[0] & 0x0F ) - 10; //JG
			        data_id = p.data[0] - segment; //JG
			}
...
void hexfile::Decode() {
	...
	for (auto p : packets) {
            ...
            else if (p.header.sensorID == 223) { // upload command dump id="DF"
		    sensor_info.parse(p.data);
            }
            else if ( ( p.data[0] & 0xF0 ) == 208 && ( p.data[0] & 0x0F ) < 10 ) { // parameter dump id="D0 etc": Exclude NOPP DA-DD : Must be listed after p.header.sensorID == 222
		    //DS CONFIG dump does not necessarily divide packets by full-width config items.  Thus packets (D0, D1, D2, etc) cannot run independently.
		    ...
		    if ( !catdata.empty() | ( p.data[lengthc-2] != 124 | p.data[lengthc-1] != 59 ) ) { 
		      ...
		      miss.parse(p.data);
	    ...
	    else if ( ( ( p.data[0] & 0xF0 ) == 80 ) && ( ( p.data[0] & 0x0F ) < 8 ) ) { // Rise, but exclude 0x58-0x5F Deep NOPP
					rise.Decode(p.data);
	    }
					...
					eng_data.parse_pfile(p.data,DS_PATH + "/config/diagnostic.json");
					//log("Packet[10] Diagnostic");
	    }
	    else if (p.header.sensorID == 226 ) { // diagnostic engineering data
					eng_data.parse_pfile(p.data,DS_PATH + "/config/Engineering_Data.json");
					//log("Packet[10] Diagnostic");
	    }
